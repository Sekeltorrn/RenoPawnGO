...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (sessionManager.isUserLoggedIn()) {
            startActivity(Intent(this, LoginSuccessSplashActivity::class.java))
            finish()
            return
        }
        ...
        viewModel.loginResult.observe(this) { resultPair ->
            ...
            if (user != null) {
                // --- THE FIX: PERSIST SESSION DATA BEFORE OTP ---
                sessionManager.saveCustomerId(user.id)
                sessionManager.saveShopCode(schemaName)
                
                viewModel.sendLoginOtp(user.email)
